...
class AMPTrainer(SimpleTrainer):
    ...
    def run_step(self,cfg):
        ...
        if cfg.DATASETS.NAMES[0].lower() in data['img_paths'][0].lower():
            ...
            raise RuntimeError('Please modify your directory name of data as intro')     
        if isinstance(self.model, DistributedDataParallel):
            models = self.model.module   
        else:
            models = self.model 
        if data_name == 'Expert1':
            self.set_bn_Expert1_train(models)
            self.set_bn_Expert2_eval(models)
            self.set_bn_Expert3_eval(models)
        elif data_name == 'Expert2':
            self.set_bn_Expert1_eval(models)
            self.set_bn_Expert2_train(models)
            self.set_bn_Expert3_eval(models)            
        elif data_name == 'Expert3':
            self.set_bn_Expert1_eval(models)
            self.set_bn_Expert2_eval(models)
            self.set_bn_Expert3_train(models)
    ...
    def set_bn_Expert1_eval(self, model):
        for name, module in model.backbone.named_children():
            if 'layer' in name:
                for name, module in module.named_children():
                    if 'downsample' in name:
                        module[2].eval()
                        for p in module[2].parameters():
                            p.requires_grad_(False)                
                    else:
                        if 'bn11' in name or 'bn21' in name or 'bn31' in name:
    ...
    def set_bn_Expert1_train(self, model):
        for name, module in model.backbone.named_children():
            if 'layer' in name:
                for name, module in module.named_children():
                    if 'downsample' in name:
    ...
    def set_bn_Expert2_eval(self, model):
        for name, module in model.backbone.named_children():
            if 'layer' in name:
                for name, module in module.named_children():
                    if 'downsample' in name:
                        module[3].eval()
                        for p in module[3].parameters():
                            p.requires_grad_(False)                
                    else:
                        if 'bn12' in name or 'bn22' in name or 'bn32' in name:
                            # Change BN in freeze layers to eval mode
                            module.eval()
                            for p in module.parameters():
                                p.requires_grad_(False)                    
    ...
    def set_bn_Expert2_train(self, model):
        for name, module in model.backbone.named_children():
            if 'layer' in name:
                for name, module in module.named_children():
                    if 'downsample' in name:
    ...
    def set_bn_Expert3_eval(self, model):
        for name, module in model.backbone.named_children():
            if 'layer' in name:
                for name, module in module.named_children():
                    if 'downsample' in name:
                        module[4].eval()
                        for p in module[4].parameters():
                            p.requires_grad_(False)                
                    else:
                        if 'bn13' in name or 'bn23' in name or 'bn33' in name:
                            # Change BN in freeze layers to eval mode
                            module.eval()
                            for p in module.parameters():
                                p.requires_grad_(False)                    
    ...
    def set_bn_Expert3_train(self, model):
        for name, module in model.backbone.named_children():
            if 'layer' in name:
                for name, module in module.named_children():
                    if 'downsample' in name:
